...
/**
 * [[ResourceCounter]] is a data structure that allows reusing one [[Resource]] multiple times
 * avoiding re-allocation.
 *
 * Resource allocation is lazy and happened on allocation of [[ResourceCounter.resource]]. If
 * [[ResourceCounter.resource]] allocated again - then the source resource will not be allocated and
 * cached value will be used. In case of concurrent allocation of [[ResourceCounter.resource]] the
 * source resource guaranteed to be allocated only once, same rule applied for releasing resource.
 * The release will happened after last [[ResourceCounter.resource]] is released.
 *
 * [[ResourceCounter]] can be reused multiple times - after is was allocated/released first time one
 * can allocate it again, sample:
 * {{{
