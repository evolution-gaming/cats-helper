 ...
 *
 * There are, currently, two ways of using the class and it is recommended to select one, describe
 * it in project coding guidelines, and stick with it to avoid a confusion for the newcomers. Both
 * ways are described below.
 *
 * =Traditional approach=
 *
 * The intented usage is to have a single instance of `LogOf` in the application and use it to
 * create `Log` instances for each class.
 *
 * The following could be written somewhere in the application initialization code such as
 * [[cats.effect.IOApp]] instance:
 * {{{
 ...
 * }}}
 *
 * The main advantage of such approach is a minimal boilerplate when passing `LogOf` around, and an
 * ability to reuse `LogOf` and `Log` instances, decreasing the performance overhead.
 *
 * =MDC-style approach=
 *
 * The alternative way to use `LogOf` is to create a child instance each time the additional
 * information should be passed to the underlying call, making it a poor's man MDC. The additonial
 * information is then to be passed using `withField` method on `LogOf` itself.
 *
 * If `LogOf` is used in such a way, it should not be passed implicitly, to avoid accidential mix up
 * of the `LogOf` instances.
 *
 * For sake of uniformity, if this approach is used, the recommended practice is to never pass `Log`
 * instance anywhere at all, creating a new one each time it is required. Saying that, it is
 * acceptable to reuse them for performance or brevity reasons.
 *
 ...
 * }}}
 *
 * This approach introduces a slight overhead, because it requires creating both `LogOf` and `Log`
 * instances on each call, but allows passing an additional context to the called methods.
 *
 * @see
 *   [[https://slf4j.org/api/org/slf4j/LoggerFactory.html LoggerFactory]] for a typical underlying
 *   implementation.
 */
trait LogOf[F[_]] {
...
object LogOf {
  ...
  implicit class LogOfOps[F[_]](val self: LogOf[F]) extends AnyVal {
    ...
    def forClass[C](
      implicit
      C: ClassTag[C],
    ): F[Log[F]] = self.apply(C.runtimeClass)

    def mapLog(
      f: Log[F] => Log[F],
    )(implicit
      F: Monad[F],
    ): LogOf[F] = new MapLog(self, f)

    def prefixed(
      prefix: String,
    )(implicit
      F: Monad[F],
    ): LogOf[F] = mapLog(_.prefixed(prefix))
    ...
    def withField(
      ...
      F: Monad[F],
    ): LogOf[F] = mapLog(_.withField(key, value))
  }
