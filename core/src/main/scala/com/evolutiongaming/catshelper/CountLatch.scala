...
import cats.effect.{Async, BracketThrow, Concurrent, Resource}
...
object CountLatch {
  ...
  def apply[F[_]: Concurrent](n: Int = 0): F[CountLatch[F]] = {
  ...
  implicit final class CountLatchOps[F[_]](val latch: CountLatch[F])
    ...
    def surround[A](fa: F[A])(implicit F: BracketThrow[F]): F[A] =
