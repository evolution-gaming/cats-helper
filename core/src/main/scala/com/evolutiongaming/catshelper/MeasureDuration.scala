...
trait MeasureDuration[F[_]] {
  ...
  /**
   * Starts measuring duration. The outer effect is used to start measuring, the inner effect is
   * used to stop measuring and get the result duration.
   */
...
object MeasureDuration {
  ...
  implicit class MeasureDurationOps[F[_]](val self: MeasureDuration[F]) extends AnyVal {
    ...
    def mapK[G[_]](
      ...
      F: FlatMap[F],
    ): MeasureDuration[G] = new MeasureDuration[G] {
