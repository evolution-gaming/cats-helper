...
trait TestFrameworkApi[F[_]] {
   ...
   *
   * @param outcome
   *   an outcome which can be non-termination, failure, or success.
   * @param tcState
   *   `TestContext` state at the moment of completion.
   */
...
object TestFrameworkApi {
  ...
  /**
   * Returns either a default API (ScalaTest as of now) if it's available on the classpath. Uses
   * [[NoFrameworkApi]] as a fallback.
   */
  def resolveDefault[F[_]](
    implicit
    l: Lookup[F],
  ): TestFrameworkApi[F] = l.instance
}
...
private[testkit] object Lookup extends LookupLowPriority {
  implicit def scalaTestIsDefault(
    implicit
    ev: TestFailedException =:= TestFailedException,
  ): Lookup[IO] = {
